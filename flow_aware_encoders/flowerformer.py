...
from torch_geometric.nn import GatedGraphConv as GatedGCNLayer
...
class FLOWERFormer(torch.nn.Module):
    ...
    def dual_forward(self, batch):
        for module in list(self.children())[:-1]:
            batch = module(batch)
        node_embeddings = batch.x.clone()
        pred_ys = list(self.children())[-1](batch)[0]
        
        return node_embeddings, pred_ys
    
    def triple_forward(self, batch):
        for module in list(self.children())[:-1]:
            batch = module(batch)
        node_embeddings = batch.x.clone()
        pred_ys1 = list(self.children())[-1](batch)[0]
        # pred_ys2 = self.add_layer(batch)
        
        return node_embeddings, pred_ys
    
    
class LP_MLP(torch.nn.Module) : 
    def __init__(self, dim_in, hidden_dim):
        super(LP_MLP, self).__init__()
        
        self.lin1 = torch.nn.Linear(dim_in, 1)
